...
#[TODO: Is there any reason *not* to rename 'Stomp' to 'Psychic Blast'? As far as I can tell, the intent seems to have been for 'Stomp' to operate in exactly the same way as 'Psychic Blast' in all respects. However, should *every* faction-specific Fate card work *only* for that faction, even in an allied force? If so then perhaps  the 'Stomp' version of 'Psychic Blast' should specify '*Ork* {psyker} units'?]#
