...
* The Defender's deployment zone extends up to half-way across the playing area from their deployment edge.
* The Attacker's deployment zone extends up to 20 cm onto the table from their deployment edge.
...
{blank}[TODO: Clarify: This scenario seems to have been written with no thought to Flyer or War Engine detachments. Should we write them out explicitly, or allow for them properly? If the latter, are there game balance issues to account for? How do Flyer detachments affect how many ground-based detachments must the Attacker get off of the Defender's table edge?]

== Play the game

== Game length and victory conditions

To win this scenario, the Attacker must exit half of the detachments in their army off of the Defender's table edge by the end of Turn 6.
